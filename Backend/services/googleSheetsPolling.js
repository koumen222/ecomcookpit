...
const POLLING_INTERVALS = {
  ...
  '1min': 60 * 1000,       // Très rapide
  '2min': 2 * 60 * 1000,   // Rapide
  '5min': 5 * 60 * 1000,   // Standard
  '15min': 15 * 60 * 1000, // Lent
  '30min': 30 * 60 * 1000, // Très lent
  '1hour': 60 * 60 * 1000  // Ultra lent
};

let pollingTimers = new Map(); // workspaceId -> timer
let isRunning = false;
let lastPollCache = new Map(); // sourceId -> { rowCount, timestamp }
  ...
  try {
    ...
    for (const source of settings.sources || []) {
      ...
      try {
        ...
        const cached = lastPollCache.get(sourceKey);
        const now = Date.now();
        
        // Skip if polled less than 30 seconds ago (rate limiting)
        ...
        if (!sheetData.rows || sheetData.rows.length === 0) {
          lastPollCache.set(sourceKey, { rowCount: 0, timestamp: now });
          continue;
        }
        ...
        // Update cache
        lastPollCache.set(sourceKey, { rowCount: currentRowCount, timestamp: now });
        ...
        if (currentRowCount > lastRowCount) {
          ...
          for (let i = 0; i < newRows.length; i++) {
            ...
            if (parsed.success && parsed.data) {
              ...
              // Check if order already exists by sheetRowId
              ...
              if (!existingOrder) {
                ...
                try {
                  ...
                  if (settings.autoSync?.notifyOnChanges !== false) {
                    ...
                    console.log(`🔔 [POLL] New order created & notified: ${newOrder.clientName} - ${newOrder.product}`);
                  }
                } catch (createError) {
                  console.error(`❌ [POLL] Error creating order from row ${rowIndex}:`, createError.message);
                }
              }
            }
          }
          
          if (newOrdersCreated > 0) {
            console.log(`✅ [POLL] ${newOrdersCreated} new orders created from ${source.name}`);
          }
