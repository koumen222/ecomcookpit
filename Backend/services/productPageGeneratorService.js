...
function languageDirective(language = 'français') {
  ...
  if (lang.startsWith('es')) return '100% ESPAÑOL PERFECTO — todo el texto para el cliente en español impecable, cero faltas de ortografía o gramática';
  return "100% FRANÇAIS PARFAIT — zéro faute d'orthographe, zéro faute de grammaire";
}
...
${premiumContract}`;
  ...
  const messages = [
    {
      role: 'system',
      content: 'Tu es le générateur premium séparé. Ta sortie est uniquement JSON. Tu crées une structure premium_product_page complète, jamais l’ancien template classique. ' + languageDirective(language) + '. Promesses crédibles. Prompts images en anglais.',
    },
    { role: 'user', content: userPrompt },
  ];
