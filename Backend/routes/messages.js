...
router.post('/:channel', requireEcomAuth, async (req, res) => {
  try {
    ...
    // ── Notifications push aux autres membres du workspace ─────────────
    try {
      ...
      // Récupérer tous les membres actifs du workspace sauf l'expéditeur
      const members = await EcomUser.find({
        workspaceId,
        isActive: true,
      ...
      await Promise.allSettled(
        members.map((member) =>
          sendPushNotificationToUser(member._id, {
