...
router.get('/:channel', requireEcomAuth, async (req, res) => {
  try {
    ...
    // Check if user is participant (if channel has participants list)
    const channelDoc = await Channel.findOne({ workspaceId, slug: channel });
    if (channelDoc && channelDoc.participants && channelDoc.participants.length > 0) {
      if (!channelDoc.hasParticipant(req.ecomUser._id)) {
        return res.status(403).json({ success: false, message: 'Vous n\'êtes pas membre de ce groupe' });
      }
    }
...
router.post('/:channel', requireEcomAuth, async (req, res) => {
  try {
    ...
    // ── Notifications push aux participants du canal uniquement ─────────────
    try {
      const senderName = req.ecomUser.name || req.ecomUser.email.split('@')[0];
      const preview = content.trim().length > 60 ? content.trim().slice(0, 60) + '…' : content.trim();

      // Get channel to access participants
      const channelDoc = await Channel.findOne({ workspaceId, slug: channel });
      
      let recipientIds = [];
      if (channelDoc && channelDoc.participants && channelDoc.participants.length > 0) {
        // Send only to participants with notifications enabled
        recipientIds = channelDoc.getNotifiableParticipants(req.ecomUser._id);
      } else {
        // Fallback: send to all workspace members (for channels without participants yet)
        const members = await EcomUser.find({
          workspaceId,
          isActive: true,
          _id: { $ne: req.ecomUser._id },
        }).select('_id').lean();
        recipientIds = members.map(m => m._id);
      }

      // Send notifications only to actual participants
      await Promise.allSettled(
        recipientIds.map((userId) =>
          sendPushNotificationToUser(userId, {
            ...
            tag: `message-${channel}`,
            data: { type: 'new_message', channel, messageId: message._id },
          })
        )
      );
    } catch (notifError) {
      // Ne pas faire échouer la requête si les notifs plantent
      console.warn('⚠️ Erreur envoi notifications message:', notifError.message);
    }
    // ───────────────────────────────────────────────────────────────────

    // ── Notification interne (panneau notifications) ─────────────────
    try {
      const senderName = req.ecomUser.name || req.ecomUser.email.split('@')[0];
      await notifyNewMessage(workspaceId, {
        senderName,
        channel,
        content: content.trim(),
      ...
      console.warn('⚠️ Erreur notification interne message:', internalNotifError.message);
    }
    // ───────────────────────────────────────────────────────────────────
