...
router.get('/', requireEcomAuth, async (req, res) => {
  try {
    ...
    const enrichedGoals = await Promise.all(goals.map(async (goal) => {
      ...
      if (goal.closeuseId) {
        matchQuery.reportedBy = goal.closeuseId;
      } else if (req.ecomUserRole === 'ecom_closeuse') {
...
router.post('/', requireEcomAuth, validateEcomAccess('admin', 'write'), async (req, res) => {
  try {
    const { type, targetValue, periodType, year, month, weekNumber, day, product, deliveryCount, closeuseId } = req.body;
    let start, end;
    const createdGoals = [];
    ...
    if (mainGoal.closeuseId) {
      matchQuery.reportedBy = mainGoal.closeuseId;
    } else if (req.ecomUserRole === 'ecom_closeuse') {
      matchQuery.reportedBy = req.user.id;
    }
