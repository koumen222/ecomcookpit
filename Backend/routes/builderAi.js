...
// ─── Clonage de page produit concurrent → fiche réécrite + images IA ─────────
// POST /builder-ai/clone-product-page { url, maxImages? } → { jobId }
// GET  /builder-ai/clone-product-page/jobs/:id → { status, step, progress, result }
...
router.get('/clone-product-page/jobs/:id', requireEcomAuth, async (req, res) => {
  const { getCloneJob } = await import('../services/productCloneService.js');
...
router.post('/clone-product-page/save', requireEcomAuth, async (req, res) => {
  try {
    const p = req.body?.product || {};
    const name = String(p.name || '').trim();
    if (!name) return res.status(400).json({ success: false, message: 'Nom du produit requis' });
    const StoreProduct = (await import('../models/StoreProduct.js')).default;
    const sp = await StoreProduct.create({
      workspaceId: req.workspaceId,
      ...(req.activeStoreId ? { storeId: req.activeStoreId } : {}),
      createdBy: req.ecomUser?._id,
      name: name.slice(0, 200),
      description: String(p.description || '').slice(0, 50000),
      price: Math.max(0, Number(p.price) || 0),
      currency: String(p.currency || 'XOF').slice(0, 8),
...
// ─── POST /builder-ai/avatar-script — monologue de l'avatar depuis le produit ─
// Body : { productName?, productContext?, productImageUrl?, durationSec? }
// → { success, script } — texte prêt à prononcer (hook → bénéfice → preuve →
// CTA commande + livraison OBLIGATOIRE). Sans description : la photo produit
// est analysée par vision pour ancrer le script sur le vrai produit.
router.post('/avatar-script', requireEcomAuth, async (req, res) => {
  try {
    ...
    if (!String(productContext).trim() && /^https?:\/\//.test(String(productImageUrl))) {
      const { analyzeProductImageForVideo } = await import('../services/openaiImageService.js');
      visual = await analyzeProductImageForVideo(String(productImageUrl)).catch(() => '');
    }
    if (!String(productName).trim() && !String(productContext).trim() && !visual) {
      return res.status(400).json({ success: false, message: 'Donne le produit : un nom, une description ou sa photo' });
    }
...
Hook choisi (première phrase du monologue) : ${hookClean}` : ''}`;
    ...
    if (script.length < 20) throw new Error('Script vide — réessaye');
    return res.json({ success: true, script: script.slice(0, 1500) });
  } catch (err) {
    console.error('[BuilderAI] avatar-script error:', err.message);
    return res.status(500).json({ success: false, message: err.message || 'Génération du script impossible' });
  }
});
...
router.post('/lipsync', requireEcomAuth, async (req, res) => {
  try {
    ...
    const { imageUrl = '', videoUrl = '', audioUrl = '', text = '', voiceRefId = '', motion = 'presenter', motionPrompt = '', tier = 'standard' } = req.body || {};
    ...
    const tierClean = ['premium', 'cinema'].includes(String(tier).toLowerCase()) ? String(tier).toLowerCase() : 'standard';
...
- Si role = cta : la phrase est un appel à l'action D'ACHAT explicite adapté au paiement à la livraison (« Commandez maintenant et faites-vous livrer chez vous », « Cliquez sur Commander, vous ne payez qu'à la livraison »…) — jamais un CTA vague.`;
    const user = `Produit : ${String(productName).slice(0, 120) || '—'}
Contexte : ${String(productContext).replace(/<[^>]+>/g, ' ').slice(0, 600) || '—'}
