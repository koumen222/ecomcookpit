...
router.get('/alerts', 
  ...
  async (req, res) => {
    try {
      ...
      const lowStockProducts = productsWithActualStock
      ...
      res.json({
        ...
        data: {
          lowStockProducts: lowStockProducts.map(p => ({
            ...p,
            urgency: p.actualStock === 0 ? 'critical' : p.actualStock <= p.reorderThreshold / 2 ? 'high' : 'medium'
          })),
...
// POST /api/ecom/stock/sync - Synchroniser Product.stock depuis StockLocation
router.post('/sync',
  requireEcomAuth,
  validateEcomAccess('products', 'write'),
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `${syncResults.length} produit(s) synchronisé(s)`,
        data: syncResults
      });
    } catch (error) {
...
async function _checkStockAlerts(product, workspaceId) {
  try {
    ...
    } else if (actualStock <= (product.reorderThreshold || 5)) {
      notifyStockLow(admin.email, { product: { ...product, stock: actualStock }, workspace, userId: admin._id }).catch(() => {});
    }
  } catch (err) {
