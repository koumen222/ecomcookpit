        ...
        <div className="bg-white rounded-2xl border border-gray-200/80 overflow-hidden shadow-sm">
          ...
          <div className="divide-y divide-gray-50 max-h-[420px] overflow-y-auto">
            {recentlyCreated.map(u => (
              <div key={u._id} className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50/80 transition-colors">
                <div className="w-9 h-9 rounded-xl bg-sky-50 flex items-center justify-center text-xs font-bold text-sky-700 flex-shrink-0 ring-1 ring-inset ring-teal-600/10">
                  {u.email?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">{u.email}</p>
