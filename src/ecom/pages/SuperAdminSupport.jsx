                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      {/* Avatar */}
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 mt-0.5 ${isActive ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                        {(conv.visitorName || conv.visitorEmail || conv.sessionId || '?').charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1 mb-0.5">
                          <span className="text-[13px] font-semibold text-gray-900 truncate">
                            {conv.visitorName || conv.visitorEmail || conv.sessionId?.slice(0, 12) + '…'}
                          </span>
            ...
            <div className="flex items-center justify-between px-5 py-3.5 bg-white border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-emerald-600 flex items-center justify-center text-white font-bold text-sm">
                  {(detail.visitorName || detail.visitorEmail || detail.sessionId || '?').charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{detail.visitorName || 'Visiteur anonyme'}</p>
                  <p className="text-[11px] text-gray-400">
                    {detail.visitorEmail && <>{detail.visitorEmail} · </>}
                    Session: <span className="font-mono">{detail.sessionId?.slice(0, 14)}…</span>
