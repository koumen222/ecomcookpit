...
const ProductFinder = () => {
  ...
  const calculateSuggestedPrice = () => {
    ...
    // Le prix ne doit JAMAIS être inférieur ù  10 000
  ...
  const validateForm = () => {
    ...
    if (!product.sourcingPrice || parseFloat(product.sourcingPrice) <= 0) {
      setError('Le prix sourcing doit être supérieur ù  0');
      return false;
    }

    if (product.sourcingType === 'china') {
      if (!product.weight || parseFloat(product.weight) <= 0) {
        setError('Le poids doit être supérieur ù  0');
        return false;
      }
      if (!product.pricePerKg || parseFloat(product.pricePerKg) <= 0) {
        setError('Le prix du kilo doit être supérieur ù  0');
        return false;
      }
    }
    
    if (!product.sellingPrice || parseFloat(product.sellingPrice) <= 0) {
      setError('Le prix de vente doit être supérieur ù  0');
      return false;
    }
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-2 top-1/2 transform -translate-y-1/2 px-2 py-1 bg-emerald-100 text-emerald-700 text-xs rounded hover:bg-emerald-200"
                  >
                    Auto
                  </button>
                </div>
                ...
                {product.pros.map((pro, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
