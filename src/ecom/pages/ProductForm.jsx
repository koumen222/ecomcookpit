      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={handleChange}
                className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-emerald-600 focus:border-emerald-600"
              />
        ...
        {(formData.sellingPrice && formData.productCost) && (
          <div className="mt-6 p-4 bg-emerald-50 rounded-lg">
            <h3 className="text-sm font-medium text-emerald-900 mb-2">Aperçu financier</h3>
            <div className="grid grid-cols-2 gap-3 sm:gap-4 text-sm">
              <div>
                <span className="text-emerald-700">Prix de vente:</span>
                <span className="ml-2 font-semibold">{fmt(formData.sellingPrice)}</span>
              </div>
              <div>
                <span className="text-emerald-700">Coût total:</span>
                <span className="ml-2 font-semibold">{fmt((parseFloat(formData.productCost) || 0) + (parseFloat(formData.deliveryCost) || 0))}</span>
              </div>
              <div className="col-span-2">
                <span className="text-emerald-700">Bénéfice estimé:</span>
            ...
            {calculateBenefit() <= 0 && (
              <div className="mt-2 text-xs text-red-600">
                âš ï¸ Attention: Ce produit n'est pas rentable selon les coûts actuels
              </div>
            )}
          </div>
        )}
