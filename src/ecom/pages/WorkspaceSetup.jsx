    ...
    <div className="min-h-screen bg-[#0F1115] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-[440px] relative">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div>
              ...
              <input type="text" required placeholder="Ex: Ma Boutique, Mon Business..." value={workspaceName} onChange={e => setWorkspaceName(e.target.value)}
                className="block w-full px-4 py-3 bg-[#1A1C22]/80 border border-[#2A2C33] rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#0F6B4F] focus:border-transparent text-sm transition" />
            </div>
            <div>
              ...
              <p className="mt-2 text-xs text-gray-600">
                {selectedRole === 'ecom_admin' 
                  ? 'En tant qu\'administrateur, vous aurez accès ù  toutes les fonctionnalités et pourrez gérer votre équipe.'
        ...
        <button onClick={() => navigate('/ecom/landing')}
          className="mt-4 w-full py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white border border-[#2A2C33] hover:border-gray-600 bg-transparent hover:bg-white/5 transition flex items-center justify-center gap-2">
