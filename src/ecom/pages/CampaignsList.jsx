...
const CampaignsList = () => {
  ...
  useEffect(() => {
    ecomApi.get('/workspaces/whatsapp-config')
      .then(res => setWaConfig(res.data.data.whatsappConfig))
      .catch(() => setWaConfig({ status: 'none', phoneNumber: '' }));
  }, []);
  ...
  const handleWaRequest = async (e) => {
    ...
    setWaLoading(true);
    try {
      const res = await ecomApi.post('/workspaces/whatsapp-request', { phoneNumber: waPhone });
      setWaConfig({ status: 'pending', phoneNumber: waPhone.replace(/\D/g, '') });
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="text-2xl font-bold">⚠️ Messages envoyés avec NOTRE numéro</h2>
                </div>
                <p className="text-white/90 text-sm mb-4 max-w-2xl">
                  <strong>Actuellement</strong>, toutes vos campagnes WhatsApp sont envoyées depuis notre numéro. Pour utiliser votre propre numéro personnel et augmenter la confiance de vos clients, postulez dès maintenant !
                </p>
                <div className="flex flex-wrap gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-1.5 text-xs bg-white/20 px-3 py-1.5 rounded-full">
                    <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3z"/></svg>
                    Taux de réponse +40%
                  </div>
                  <div className="flex items-center gap-1.5 text-xs bg-white/20 px-3 py-1.5 rounded-full">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Link 
                    ...
                    className="px-6 py-3 bg-white text-green-600 font-bold rounded-lg hover:bg-gray-100 transition shadow-lg text-center"
                  >
                    📋 Postuler pour mon numéro WhatsApp
                  </Link>
                  <Link 
                    to="/ecom/whatsapp-en-savoir-plus" 
                    className="px-6 py-3 bg-white/20 text-white font-semibold rounded-lg hover:bg-white/30 transition border border-white/30 text-center"
                  >
                    En savoir plus
                  </Link>
                </div>
      ...
      {waConfig && waConfig.status === 'pending' && (
        <div className="mb-6 bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl shadow-xl p-6 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
          <div className="relative z-10">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-xl font-bold mb-2">📋 Configuration en cours — +{waConfig.phoneNumber}</h3>
                <p className="text-white/90 text-sm mb-3">
                  Notre équipe vous contactera dans les plus brefs délais pour finaliser l'activation de votre numéro WhatsApp personnel.
                </p>
                <div className="bg-white/10 backdrop-blur rounded-lg p-3">
                  <p className="text-xs text-white/80">
                    <strong>⏰ Temps estimé :</strong> Quelques heures<br/>
                    <strong>📧 Vous serez contacté par :</strong> WhatsApp ou Email<br/>
                    <strong>🚀 En attendant :</strong> Vos campagnes utilisent le numéro par défaut
                  </p>
                </div>
              </div>
            </div>
      ...
      {waConfig && waConfig.status === 'active' && (
        <div className="mb-6 bg-gradient-to-r from-green-500 to-emerald-500 rounded-2xl shadow-xl p-6 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
          <div className="relative z-10">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-xl font-bold mb-1">✅ Numéro WhatsApp Activé</h3>
                <p className="text-white/90 text-sm">
                  Vos campagnes sont envoyées depuis votre numéro personnel : <span className="font-semibold">+{waConfig.phoneNumber}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {waConfig?.status === 'pending' ? (
          <div className="flex flex-col items-end gap-1">
            <button disabled className="px-4 py-2 bg-amber-100 text-amber-600 border border-amber-300 rounded-lg text-sm font-medium flex items-center gap-1.5 cursor-not-allowed opacity-80">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              Campagne bloquée
            </button>
            <p className="text-[10px] text-amber-600 font-medium">⏳ Postulation WhatsApp en attente d'approbation</p>
          </div>
        ) : (
