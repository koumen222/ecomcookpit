...
const Metric = ({label, value, sub, icon, color='text-gray-900', subColor, iconBg='bg-gray-100'}) => (
  <Card className="p-4 sm:p-5">
    <div className="flex items-start gap-3.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className={`text-xl sm:text-2xl font-bold mt-0.5 truncate ${color}`}>{value ?? '—'}</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6">
        ...
        {/* Navigation */}
        <div className="flex gap-0.5 mb-5 bg-white rounded-xl border border-gray-200/60 p-1 overflow-x-auto no-scrollbar">
          {NAV.map(n=>(
        ...
        {loading ? (
          ...
          <>
            {tab === 'overview' && <OverviewTab summary={summary} budgets={budgets} budgetSummary={budgetSummary} forecast={forecast} fmt={fmt} setTab={setTab} periodLabel={periodLabel} pStart={pStart} pEnd={pEnd}/>}
            {tab === 'transactions' && <TransactionsTab transactions={transactions} summary={summary} balance={bal} filters={filters} setFilters={setFilters} handleDelete={handleDelete} fmt={fmt} periodLabel={periodLabel}/>}
            {tab === 'budgets' && <BudgetsTab budgets={budgets} budgetSummary={budgetSummary} showBudgetForm={showBudgetForm} setShowBudgetForm={setShowBudgetForm} editingBudget={editingBudget} setEditingBudget={setEditingBudget} budgetForm={budgetForm} setBudgetForm={setBudgetForm} handleBudgetSubmit={handleBudgetSubmit} handleDeleteBudget={handleDeleteBudget} products={products} fmt={fmt} budgetMonth={budgetMonth} setBudgetMonth={setBudgetMonth} loadProducts={loadProducts}/>}
            {tab === 'analyse' && <AnalyseTab accountingSummary={accountingSummary} fmt={fmt} periodLabel={periodLabel} pStart={pStart} pEnd={pEnd}/>}
            {tab === 'previsions' && <PrevisionsTab forecast={forecast} fmt={fmt}/>}
          </>
        )}
      </div>
    </div>
...
/* ═══════════════════════════════════════════
   OverviewTab
   ═══════════════════════════════════════════ */
const OverviewTab = ({ summary, budgets, forecast, fmt, setTab, periodLabel, pStart, pEnd }) => {
  const bal = (summary.totalIncome||0) - (summary.totalExpense||0);
  const f = forecast;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric label="Entrées" value={fmt(summary.totalIncome)} sub={`${summary.incomeCount||0} transactions`} icon={I.trend} color="text-emerald-600" iconBg="bg-emerald-50"/>
        <Metric label="Dépenses" value={fmt(summary.totalExpense)} sub={`${summary.expenseCount||0} transactions`} icon={I.down} color="text-red-500" iconBg="bg-red-50"/>
        <Metric label="Solde net" value={fmt(bal)} sub={bal>=0?'Excédentaire':'Déficitaire'} icon={I.wallet} color={bal>=0?'text-emerald-600':'text-red-500'} iconBg={bal>=0?'bg-emerald-50':'bg-red-50'}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-5 mb-4">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-2 space-y-1 text-xs text-gray-400">
                <div className="flex justify-between gap-4"><span>Dép./jour</span><span className="text-red-500 font-semibold">{fmt(f.dailyExpenseRate)}</span></div>
                <div className="flex justify-between gap-4"><span>Ent./jour</span><span className="text-emerald-600 font-semibold">{fmt(f.dailyIncomeRate)}</span></div>
              </div>
            </div>
          </div>
        ...
        <Card className="p-5">
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            ...
            <div className="text-center bg-gray-50 rounded-lg p-3">
              <p className="text-xl font-bold text-emerald-600">{fmt(orders.revenueThisMonth)}</p>
              ...
              {topBudgets.map(b=>{
                ...
                return (
                  <div key={b._id}>
                    ...
                    <div className="flex justify-between text-[10px] text-gray-400 mt-1"><span>{fmt(b.totalSpent)}</span><span>{fmt(b.amount)}</span></div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              {label:'Dépenses', value:fmt(f.projectedExpense), vs:f.expenseVsAvg, color:'text-red-500', bg:'bg-red-50', inv:true},
              {label:'Entrées', value:fmt(f.projectedIncome), vs:f.incomeVsAvg, color:'text-emerald-600', bg:'bg-emerald-50'},
              {label:'Solde', value:fmt(f.projectedBalance), color:(f.projectedBalance||0)>=0?'text-emerald-600':'text-red-500', bg:(f.projectedBalance||0)>=0?'bg-emerald-50':'bg-red-50', sub:`${f.daysLeft||0}j restants`},
            ].map((p,idx)=>(
...
/* ═══════════════════════════════════════════
   TransactionsTab
   ═══════════════════════════════════════════ */
const TransactionsTab = ({ transactions, summary, balance, filters, setFilters, handleDelete, fmt, periodLabel }) => (
  <div className="space-y-4">
    <div className="grid grid-cols-3 gap-3">
      <Metric label="Entrées" value={fmt(summary.totalIncome)} sub={`${summary.incomeCount||0} opérations`} icon={I.trend} color="text-emerald-600" iconBg="bg-emerald-50"/>
      <Metric label="Dépenses" value={fmt(summary.totalExpense)} sub={`${summary.expenseCount||0} opérations`} icon={I.down} color="text-red-500" iconBg="bg-red-50"/>
      <Metric label="Solde net" value={fmt(balance)} sub={balance>=0?'Excédentaire':'Déficitaire'} icon={I.wallet} color={balance>=0?'text-emerald-600':'text-red-500'} iconBg={balance>=0?'bg-emerald-50':'bg-red-50'}/>
    </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-100">
          <thead><tr className="bg-gray-50/80">
            {['Date','Type','Catégorie','Description','Montant',''].map((h,i)=>(
              <th key={i} className={`px-4 py-3 text-[11px] font-bold text-gray-400 uppercase tracking-wider ${i>=4?'text-right':'text-left'} ${i===2?'hidden sm:table-cell':''} ${i===3?'hidden md:table-cell':''}`}>{h}</th>
            ))}
          </tr></thead>
          <tbody className="divide-y divide-gray-50">
            {transactions.length===0 ? (
              ...
              <tr key={tx._id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell font-medium">{CAT[tx.category]||tx.category}</td>
                <td className="px-4 py-3 text-sm text-gray-400 max-w-[200px] truncate hidden md:table-cell">{tx.description||'—'}</td>
                <td className={`px-4 py-3 text-sm font-bold text-right tabular-nums ${tx.type==='income'?'text-emerald-600':'text-red-500'}`}>{tx.type==='income'?'+':'-'}{fmt(tx.amount)}</td>
...
const BudgetsTab = ({ budgets, budgetSummary, showBudgetForm, setShowBudgetForm, editingBudget, setEditingBudget, budgetForm, setBudgetForm, handleBudgetSubmit, handleDeleteBudget, products, fmt, budgetMonth, setBudgetMonth, loadProducts }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric label="Budget total" value={fmt(budgetSummary.totalBudget)} icon={I.target} color="text-gray-900" iconBg="bg-gray-100"/>
        <Metric label="Dépensé" value={fmt(budgetSummary.totalSpent)} icon={I.down} color="text-red-500" iconBg="bg-red-50"/>
        <Metric label="Restant" value={fmt(budgetSummary.totalRemaining)} icon={I.wallet} color={(budgetSummary.totalRemaining||0)>=0?'text-emerald-600':'text-red-500'} iconBg={(budgetSummary.totalRemaining||0)>=0?'bg-emerald-50':'bg-red-50'}/>
            ...
            return (
              <Card key={b._id} className="p-4 sm:p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="text-[10px] text-gray-400">{fmt(Math.max(b.remaining,0))} restants — {b.transactionCount||0} tx</p>
                    </div>
                ...
                <div className="flex justify-between mt-1.5">
                  ...
                  <p className="text-[10px] text-gray-400 font-medium sm:hidden">{fmt(b.totalSpent)} / {fmt(b.amount)}</p>
                </div>
              </Card>
            );
          })}
...
const AnalyseTab = ({ accountingSummary, fmt, periodLabel, pStart, pEnd }) => {
  ...
  return (
    <div className="space-y-4">
      {/* KPI row */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric label="Total entrées" value={fmt(a.totalIncome)} icon={I.trend} color="text-emerald-600" iconBg="bg-emerald-50"/>
        <Metric label="Total dépenses" value={fmt(a.totalExpenses)} icon={I.down} color="text-red-500" iconBg="bg-red-50"/>
        <Metric label="Solde global" value={fmt(a.balance)} icon={I.wallet} color={(a.balance||0)>=0?'text-emerald-600':'text-red-500'} iconBg={(a.balance||0)>=0?'bg-emerald-50':'bg-red-50'}/>
        <Metric label="Mois précédent" value={fmt(lastBal)} sub={lastBal>=0?'Excédentaire':'Déficitaire'} icon={I.cal} color={lastBal>=0?'text-emerald-600':'text-red-500'} iconBg="bg-gray-100"/>
      </div>
                ...
                return (
                  <div key={idx}>
                    <div className="flex justify-between text-sm mb-1.5">
                      ...
                      <span className="font-bold text-red-500 tabular-nums">{fmt(c.total)} <span className="text-gray-400 font-normal text-[10px]">({pct.toFixed(0)}%)</span></span>
                    </div>
              ...
              {income.sort((x,y)=>y.total-x.total).map((c,idx)=>{
                ...
                return (
                  <div key={idx}>
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="text-gray-700 font-medium">{CAT[c._id.category]||c._id.category}</span>
                      <span className="font-bold text-emerald-600 tabular-nums">{fmt(c.total)} <span className="text-gray-400 font-normal text-[10px]">({pct.toFixed(0)}%)</span></span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-1.5">
            ...
            {monthLabels.slice(-6).map(ml=>{
              ...
              return (
                <div key={ml} className="flex items-center gap-3">
                  ...
                  <span className={`text-xs font-bold w-24 text-right tabular-nums ${mBal>=0?'text-emerald-600':'text-red-500'}`}>{mBal>=0?'+':''}{fmt(mBal)}</span>
                </div>
              );
            })}
                ...
                {monthLabels.map(ml=>{
                  ...
                  return (
                    <tr key={ml} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-2.5 text-right text-emerald-600 font-semibold tabular-nums">{fmt(inc)}</td>
                      <td className="py-2.5 text-right text-red-500 font-semibold tabular-nums">{fmt(exp)}</td>
                      <td className={`py-2.5 text-right font-bold tabular-nums ${mBal>=0?'text-emerald-600':'text-red-500'}`}>{fmt(mBal)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            {raw && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <Card className="p-3.5 text-center">
                  ...
                  <p className="text-2xl font-black tabular-nums mt-0.5 text-gray-900">{fmt(raw.burnRate)}</p>
                </Card>
                <Card className="p-3.5 text-center">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {det.productData.map((p,i)=>(
                          <tr key={i} className="hover:bg-gray-50/50">
                            ...
                            <td className="py-2 px-2 text-right tabular-nums font-semibold text-gray-900">{fmt(p.margin)}</td>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {det.cityData.map((c,i)=>(
                          <tr key={i} className="hover:bg-gray-50/50">
                            ...
                            <td className="py-2 px-2 text-right tabular-nums font-semibold text-gray-900">{fmt(c.revenue)}</td>
...
/* ═══════════════════════════════════════════
   PrevisionsTab
   ═══════════════════════════════════════════ */
const PrevisionsTab = ({ forecast, fmt }) => {
  const f = forecast;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Metric label="Dép. projetées" value={fmt(f.projectedExpense)} sub={f.expenseVsAvg!==undefined?`${arr(f.expenseVsAvg)} ${Math.abs(f.expenseVsAvg)}% vs moy.`:null} icon={I.down} color="text-red-500" subColor={varColorInv(f.expenseVsAvg||0)} iconBg="bg-red-50"/>
        <Metric label="Ent. projetées" value={fmt(f.projectedIncome)} sub={f.incomeVsAvg!==undefined?`${arr(f.incomeVsAvg)} ${Math.abs(f.incomeVsAvg)}% vs moy.`:null} icon={I.trend} color="text-emerald-600" subColor={varColor(f.incomeVsAvg||0)} iconBg="bg-emerald-50"/>
        <Metric label="Solde projeté" value={fmt(f.projectedBalance)} sub={`Marge: ${f.projectedIncome>0?Math.round((f.projectedIncome-(f.projectedExpense||0))/f.projectedIncome*100):0}%`} icon={I.wallet} color={(f.projectedBalance||0)>=0?'text-emerald-600':'text-red-500'} iconBg={(f.projectedBalance||0)>=0?'bg-emerald-50':'bg-red-50'}/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-5">
          ...
          <div className="flex items-center gap-5 mb-4">
            ...
            <div className="min-w-0">
              <p className={`text-base font-bold ${scoreColor}`}>{f.healthLabel||'—'}</p>
              <div className="mt-2 space-y-1 text-xs text-gray-400">
                <div className="flex justify-between gap-4"><span>Moy. dép. 3m</span><span className="text-gray-700 font-semibold tabular-nums">{fmt(f.avg3mExpense)}</span></div>
                <div className="flex justify-between gap-4"><span>Moy. ent. 3m</span><span className="text-gray-700 font-semibold tabular-nums">{fmt(f.avg3mIncome)}</span></div>
              </div>
            </div>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2 mb-4">
            <div className="flex justify-between items-center text-sm bg-red-50 rounded-lg px-3 py-2"><span className="text-gray-600 font-medium">Dépenses / jour</span><span className="font-bold text-red-500 tabular-nums">{fmt(f.dailyExpenseRate||0)}</span></div>
            <div className="flex justify-between items-center text-sm bg-emerald-50 rounded-lg px-3 py-2"><span className="text-gray-600 font-medium">Entrées / jour</span><span className="font-bold text-emerald-600 tabular-nums">{fmt(f.dailyIncomeRate||0)}</span></div>
          </div>
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="text-center bg-gray-50 rounded-lg p-3">
              <p className="text-xl font-bold text-emerald-600 tabular-nums">{fmt(orders.revenueThisMonth)}</p>
                ...
                <tbody className="divide-y divide-gray-50">
                  {cats.map((c,idx)=>(
                    <tr key={idx} className="hover:bg-gray-50/50">
                      ...
                      <td className="py-2.5 text-right text-red-500 font-semibold text-xs tabular-nums">{fmt(c.currentSpent)}</td>
                      <td className={`py-2.5 text-right font-bold text-xs ${varColorInv(c.variation)}`}>{arr(c.variation)} {Math.abs(c.variation)}%</td>
                      <td className="py-2.5 text-right text-gray-700 font-medium text-xs tabular-nums">{fmt(c.projected)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-gray-50">
                  {prods.slice(0,8).map((p,idx)=>(
                    <tr key={idx} className="hover:bg-gray-50/50">
                      ...
                      <td className="py-2.5 text-right text-xs text-emerald-600 font-semibold tabular-nums">{fmt(p.revenue)}</td>
                      <td className="py-2.5 text-right text-xs text-gray-500">{p.deliveryRate}%</td>
                      <td className={`py-2.5 text-right text-xs font-bold tabular-nums ${p.estimatedProfit>=0?'text-emerald-600':'text-red-500'}`}>{fmt(p.estimatedProfit)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {alerts.map((a,idx)=>{
              ...
              return (
                <div key={idx} className={`border rounded-lg p-3.5 flex items-center justify-between ${sev}`}>
                  ...
                  <p className="text-sm font-bold flex-shrink-0 ml-4 tabular-nums">{fmt(a.spent)} / {fmt(a.amount)}</p>
                </div>
              );
            })}
          </div>
          ...
          <Card className="p-5">
            ...
            <div className="space-y-2">
              {weekly.map((w,idx)=>{
                ...
                return (
                  <div key={idx}>
                    <div className="flex justify-between text-[10px] text-gray-400 mb-1 font-medium"><span>Sem. {w.week}</span><span className={`font-bold ${w.balance>=0?'text-emerald-600':'text-red-500'}`}>{w.balance>=0?'+':''}{fmt(w.balance)}</span></div>
                  ...
                  {monthly.map((m,idx)=>{
                    ...
                    return (
                      <tr key={idx} className="hover:bg-gray-50/50">
                        ...
                        <td className="py-2.5 text-right text-emerald-600 font-semibold text-xs tabular-nums">{fmt(m.income)}</td>
                        <td className="py-2.5 text-right text-red-500 font-semibold text-xs tabular-nums">{fmt(m.expenses)}</td>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {(orders.byStatus||[]).map((s,idx)=>(
              <div key={idx} className="text-center p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-[10px] text-gray-400 tabular-nums">{fmt(s.revenue)}</p>
              </div>
            ))}
          </div>
        </Card>
