  ...
  <div className="group relative bg-white rounded-2xl border border-slate-200/60 p-6 transition-all duration-300 hover:shadow-xl hover:shadow-slate-900/5 hover:border-slate-300 hover:-translate-y-1 overflow-hidden">
    ...
    <div className="relative">
      <div className="flex items-start justify-between mb-3">
        ...
        {Icon && <Icon className="w-4 h-4 text-slate-400 group-hover:text-emerald-600 transition-colors" />}
      </div>
      ...
      <div className="flex items-center justify-between min-h-[20px]">
        ...
        {trend !== undefined && trend !== null && (
          <span className={`inline-flex items-center gap-1 text-xs font-bold ${trendUp ? 'text-emerald-600' : 'text-amber-500'}`}>
  ...
  <div className="flex-1 min-w-[160px]">
    ...
    {!isLast && dropRate !== undefined && (
      <div className="flex items-center justify-center my-3">
        <div className="flex items-center gap-1.5 text-xs font-bold text-amber-500 bg-amber-50 px-3 py-1.5 rounded-full border border-amber-200">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <select value={range} onChange={(e) => setRange(e.target.value)}
              className="px-4 py-2.5 bg-white border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:border-emerald-600 cursor-pointer transition-all">
        ...
        <div>
          <SectionTitle icon={TrendingUp} title="KPIs Croissance" subtitle="Acquisition, engagement & retention utilisateurs" gradient="from-emerald-700 to-emerald-700" />
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <KpiCard
              ...
              icon={Activity}
            />
            <KpiCard value={`${activationRate}%`} label="Taux d'Activation" accent="text-teal-600" sub={`${stats.totalActive || 0} / ${stats.totalUsers || 0}`} icon={Target} />
            <KpiCard value={`${kpis.retention7d ?? 0}%`} label="Retention 7j" accent="text-teal-600" sub="Utilisateurs retenus" icon={RotateCcw} />
            <KpiCard value={`${churnRate}%`} label="Taux de Churn" accent="text-amber-600" sub={`${neverLoggedIn} jamais connectes`} icon={TrendingDown} />
          </div>
        </div>

        <div>
          <SectionTitle icon={MousePointerClick} title="KPIs Conversion" subtitle="Funnel reel: visiteurs → inscrits → actives → workspace" gradient="from-amber-600 to-amber-600" />
          {funnelSteps.length > 0 ? (
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <KpiCard value={`${kpis.conversionSignup ?? 0}%`} label="Visites → Inscriptions" accent="text-emerald-700" icon={MousePointerClick} />
                <KpiCard value={`${kpis.conversionActivation ?? 0}%`} label="Inscrits → Workspace" accent="text-teal-600" icon={CheckCircle2} />
        ...
        <div>
          <SectionTitle icon={BarChart3} title="KPIs Trafic & Engagement" subtitle="Pages vues, duree de session, activite" gradient="from-teal-600 to-emerald-600" />
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            <KpiCard value={kpis.totalPageViews ?? 0} label="Pages Vues" accent="text-slate-900" icon={Eye} />
            <KpiCard value={`${kpis.avgSessionDuration ?? 0}s`} label="Duree Moy. Session" accent="text-teal-600" icon={Clock} />
            <KpiCard value={kpis.uniqueVisitors ?? 0} label="Visiteurs Uniques" accent="text-emerald-700" icon={Users} />
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            <KpiCard value={workspaces.length} label="Workspaces" accent="text-emerald-700" sub={`${activeWs} actifs`} icon={Building2} />
            <KpiCard value={totalMembers} label="Membres Total" accent="text-teal-600" sub={`${workspaces.length > 0 ? (totalMembers / workspaces.length).toFixed(1) : 0} moy/ws`} icon={Users} />
            ...
            <KpiCard value={secStats.failedLoginsLast24h ?? 0} label="Echecs Login 24h" accent="text-amber-600" icon={AlertCircle} />
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {Object.entries(roleLabels).map(([role, label]) => {
              ...
              const colors = {
                super_admin: { bg: 'bg-amber-50 border-amber-200', bar: 'bg-amber-500', text: 'text-amber-700', icon: Crown },
                ecom_admin: { bg: 'bg-emerald-50 border-emerald-200', bar: 'bg-emerald-600', text: 'text-emerald-800', icon: Briefcase },
                ecom_closeuse: { bg: 'bg-sky-50 border-sky-200', bar: 'bg-teal-500', text: 'text-sky-700', icon: Package },
        ...
        {dailySignups.length > 0 && (
          <div className="rounded-2xl border-2 border-slate-200 bg-white p-6 shadow-lg">
            <div className="flex items-center justify-between mb-5">
              ...
              <Badge className="bg-emerald-50 text-emerald-800 ring-emerald-700/20" icon={TrendingUp}>{dailySignups.length} jours</Badge>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-100">
              <div className="text-center">
                <p className="text-2xl font-black text-emerald-700">{dailySignups.reduce((s, d) => s + d.count, 0)}</p>
              ...
              <div className="text-center">
                <p className="text-2xl font-black text-teal-600">{Math.round(dailySignups.reduce((s, d) => s + d.count, 0) / dailySignups.length)}</p>
        ...
        <div className="rounded-2xl border-2 border-slate-200 bg-white p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-50 border-2 border-emerald-200 p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
              <div className="flex items-center gap-2 mb-3">
                <TrendingUp className="w-5 h-5 text-emerald-700" />
                <p className="text-xs font-black text-emerald-800 uppercase tracking-wider">Croissance</p>
              </div>
            ...
            <div className="rounded-2xl bg-gradient-to-br from-sky-50 to-emerald-50 border-2 border-sky-200 p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
              <div className="flex items-center gap-2 mb-3">
