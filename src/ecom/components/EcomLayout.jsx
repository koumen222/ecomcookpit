...
const EcomLayoutComponent = ({ children }) => {
  ...
  const mainNav = useMemo(() => [
    ...
    {
      name: 'Ma Boutique', shortName: 'Boutique', href: '/ecom/store', primary: true,
      roles: ['ecom_admin'],
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-1 w-56 rounded-xl shadow-xl border overflow-hidden z-50 bg-white border-gray-200">
                  ...
                  <div className="border-t py-1 border-gray-100">
                    {isImpersonating ? (
                      <button onClick={() => { setUserMenuOpen(false); stopImpersonation(); }} className="flex items-center gap-2 px-4 py-2 text-sm text-[#0F6B4F] hover:bg-[#0F6B4F]/5 w-full">
