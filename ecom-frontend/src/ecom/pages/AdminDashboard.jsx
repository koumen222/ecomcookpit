        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6 overflow-hidden">
            <div className="flex items-center justify-between mb-4 sm:mb-6 gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-gray-500 hidden sm:block">Performance par marge générée</p>
              </div>
              <Link to="/ecom/products" className="text-xs sm:text-sm text-blue-600 hover:text-blue-700 font-medium whitespace-nowrap flex-shrink-0">
                Voir tout →
              </Link>
            </div>
            <div className="space-y-2 sm:space-y-3">
              {stats.products.slice(0, 5).map((product, i) => {
                ...
                return (
                  <div key={product._id} className="flex items-center gap-2.5 sm:gap-4 p-2.5 sm:p-3 rounded-xl hover:bg-gray-50 transition">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-bold text-xs sm:text-sm flex-shrink-0">
                      {i + 1}
                    </div>
                    <div className="flex-1 min-w-0 overflow-hidden">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <p className="font-medium text-sm text-gray-900 truncate">{product.name}</p>
                        <span className={`px-1.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-medium border whitespace-nowrap flex-shrink-0 ${getStatusColor(product.status)}`}>
                          {product.status}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mt-0.5 text-[10px] sm:text-xs text-gray-500 truncate">
                        <span>Stock: {product.stock}</span>
                        <span>•</span>
                        <span className={`truncate ${margin >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          Marge: {fmt(margin)}
                        </span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="text-xs sm:text-sm font-bold text-gray-900 whitespace-nowrap">{fmt(product.sellingPrice || 0)}</p>
                      <p className="text-[10px] sm:text-xs text-gray-500">{marginPercent.toFixed(0)}% marge</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6 overflow-hidden">
            ...
            <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-100">
              <Link to="/ecom/stock" className="flex items-center justify-center gap-2 text-xs sm:text-sm text-blue-600 hover:text-blue-700 font-medium py-2 rounded-lg hover:bg-blue-50 transition">
