...
// Déterminer le chemin vers Backend
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const backendPath = join(__dirname, '..', 'Backend');

// Import des modèles depuis Backend
const Workspace = (await import(`file:///${join(backendPath, 'models/Workspace.js').replace(/\\/g, '/')}`)).default;
const StoreProduct = (await import(`file:///${join(backendPath, 'models/StoreProduct.js').replace(/\\/g, '/')}`)).default;
const StoreOrder = (await import(`file:///${join(backendPath, 'models/StoreOrder.js').replace(/\\/g, '/')}`)).default;
const EcomUser = (await import(`file:///${join(backendPath, 'models/EcomUser.js').replace(/\\/g, '/')}`)).default;
const AnalyticsEvent = (await import(`file:///${join(backendPath, 'models/AnalyticsEvent.js').replace(/\\/g, '/')}`)).default;
const AnalyticsSession = (await import(`file:///${join(backendPath, 'models/AnalyticsSession.js').replace(/\\/g, '/')}`)).default;
const Client = (await import(`file:///${join(backendPath, 'models/Client.js').replace(/\\/g, '/')}`)).default;
const Order = (await import(`file:///${join(backendPath, 'models/Order.js').replace(/\\/g, '/')}`)).default;
const Product = (await import(`file:///${join(backendPath, 'models/Product.js').replace(/\\/g, '/')}`)).default;
const Campaign = (await import(`file:///${join(backendPath, 'models/Campaign.js').replace(/\\/g, '/')}`)).default;
const EmailCampaign = (await import(`file:///${join(backendPath, 'models/EmailCampaign.js').replace(/\\/g, '/')}`)).default;
const WhatsAppLog = (await import(`file:///${join(backendPath, 'models/WhatsAppLog.js').replace(/\\/g, '/')}`)).default;
const Notification = (await import(`file:///${join(backendPath, 'models/Notification.js').replace(/\\/g, '/')}`)).default;
const Transaction = (await import(`file:///${join(backendPath, 'models/Transaction.js').replace(/\\/g, '/')}`)).default;

// Configuration
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/ecom-cockpit';
...
async function deleteAllStores() {
  ...
  const workspaces = await Workspace.find({}, '_id name slug').lean();
